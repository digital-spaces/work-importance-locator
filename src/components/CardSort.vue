...
//! Fix the program to re-render the components. I'm not sure exactly how we go about that, possibly by keying the columns to lists that we change.
</script>
...
<template>
    <div id="columns-container">
        ...
        <div className='col' :rank=5 @drop="onDrop($event, 5)" @dragover.prevent @dragenter.prevent>
            <Card v-for="item in rankFive" :id="item.letter" :desc="item.desc" :key="item" :card="item.id" />
        </div>
        <div className='col' :rank=4 @drop="onDrop($event, 4)" @dragover.prevent @dragenter.prevent>
            <Card v-for="item in rankFour" :id="item.letter" :desc="item.desc" :key="item" :card="item.id" />
        </div>
        <div className='col' :rank=3 @drop="onDrop($event, 3)" @dragover.prevent @dragenter.prevent>
            <Card v-for="item in rankThree" :id="item.letter" :desc="item.desc" :key="item" :card="item.id" />
        </div>
        <div className='col' :rank=2 @drop="onDrop($event, 2)" @dragover.prevent @dragenter.prevent>
            <Card v-for="item in rankTwo" :id="item.letter" :desc="item.desc" :key="item" :card="item.id" />
        </div>
        <div className='col' :rank=1 @drop="onDrop($event, 1)" @dragover.prevent @dragenter.prevent>
            <Card v-for="item in rankOne" :id="item.letter" :desc="item.desc" :key="item" :card="item.id" />
        </div>
